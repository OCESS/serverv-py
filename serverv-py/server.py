...
- Spin up an event loop that will asynchronously
  update legacy QB clients (logic for that in qb_communication) and
  send network updates to Ivan's piloting client over sockets
"""
import argparse
import asyncio
import signal
...
qb_communication.parse_sevpath(args.sevpath)
...
def update_orbit_files(event_loop):
    ...
    qb_communication.perform_qb_communication()
...
piloting = piloting_state.Piloting(qb_communication._client_path['flight'])
